...
- **Retry Mechanisms:** Retries occur in multiple interactions: \
    - Client to Cluster Manager (CM): Clients retry requests to the CM if they fail to receive server details. \
    - Client to Server: Clients retry requests to servers in case of failures. \
    - Server to CM: Servers retry registration attempts with the CM until successful. \
    - Tracking Mechanism: The CM maintains a boolean tracker to monitor server registration status and ensure all servers are properly registered before responding to clients. \
    - Retry Interval: The retry interval is currently configured as 3 seconds to balance responsiveness and avoid excessive load.
